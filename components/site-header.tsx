...
export function SiteHeader() {
  ...
  // Отслеживаем скролл для фона шапки
  useEffect(() => {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-colors duration-500 ${
        scrolled || isOpen ? 'bg-background/95 backdrop-blur-md' : 'bg-transparent'
      }`}
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 md:px-12 md:py-6">
        <Link href="/" className="z-50 flex items-center" onClick={() => setIsOpen(false)}>
          <Image
            src="/logo.png"
        ...
        <button
          ...
          className="z-50 p-2 text-foreground focus:outline-none lg:hidden"
      ...
      {/* Выпадающее мобильное меню (оверлей) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute inset-x-0 top-full flex h-screen flex-col overflow-y-auto bg-background/95 px-6 pb-32 pt-8 shadow-2xl backdrop-blur-md lg:hidden"
          >
            <nav className="flex flex-col gap-6">
              ...
              <div className="mt-10 border-t border-border/60 pt-10">
                <a
                  href="#contacts"
                  onClick={() => setIsOpen(false)}
                  className="inline-flex h-14 w-full items-center justify-center rounded-full bg-rose-400 px-6 text-lg tracking-wide text-white transition-all hover:bg-rose-500 shadow-lg shadow-rose-400/20"
                >
                  Оформить заказ
                </a>
              </div>
